...
while True:
  preset = input("do you want the the preset cycles?\n").strip().lower()
  if preset == "yes":
    break
  elif preset == "no":
    while True:
    ...
    while True:
      try:
        option = (input("Do you want the default settings for exhale time or you wnat to change it? Yes for change, No for the default settings.\n")).strip().lower()
        if option == "yes":
          while True:
            try:
